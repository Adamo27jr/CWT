...
def analyze_kh_case_with_plots(case_name, data_path, max_time=50):
    ...
    # PATTERN DE FICHIER POUR B_0_05 (cas unique, pas de x/y)
...
# ANALYSE DU CAS B_0_05 AVEC TOUTES LES VISUALISATIONS
print("🚀 DÉBUT DE L'ANALYSE COMPLÈTE DU CAS B_0_05")

# Cas B_0_05 (cas unique, pas de sous-dossiers x/y)
case_05_path = r'C:\Users\user\Desktop\Donnees_KVI\Kelvin_Helmholtz_Instabilities_B_0_05\MHD_data_kelvin_helmholtz_B_0_05'
results_05 = analyze_kh_case_with_plots('B_0_05', case_05_path, max_time=40)

# RÉSUMÉ FINAL
print(f"\n{'='*80}")
print(f"=== RÉSUMÉ FINAL DU CAS B_0_05 ===")
print(f"{'='*80}")

if results_05:
    print(f"\n🎯 CARACTÉRISTIQUES DU CAS B_0_05:")
    print(f"  - Champ magnétique: B₀ = 0.05 (FAIBLE)")
    print(f"  - Configuration: CAS UNIQUE (pas de directionnalité x/y)")
    print(f"  - Fichiers analysés: {len(results_05['all_times']) if results_05['all_times'] else 0}")

    if results_05['evolution_metrics']:
        final_metrics = results_05['evolution_metrics'][-1]
        initial_metrics = results_05['evolution_metrics'][0]
        ...
        if len(results_05['evolution_metrics']) > 1:
    ...
    print(f"  - Champ magnétique TRÈS FAIBLE (B₀ = 0.05)")
    print(f"  - Instabilité proche du cas hydrodynamique")
    print(f"  - Développement potentiellement plus libre")
    print(f"  - Référence pour comparaisons avec B₀ plus élevés")

else:
    print("❌ Échec de l'analyse du cas B_0_05")

print(f"\n✅ ANALYSE COMPLÈTE AVEC VISUALISATIONS TERMINÉE POUR LE CAS B_0_05")
