...
def analyze_kh_case_with_plots(case_name, data_path, max_time=50):
    ...
    # PATTERN DE FICHIER POUR B_0_15 (cas unique, pas de x/y)
    ...
    # Trouver les fichiers disponibles avec le bon pattern
    available_times = []
    ...
    print(f"Fichiers disponibles: {len(available_times)} (t=0 à t={max(available_times) if available_times else 'N/A'})")

    if not available_times:
        print("❌ Aucun fichier temporel trouvé")
        return None
    ...
    if len(evolution_metrics) > 0:
        ...
        print(f"\n📊 PHASES DE DÉVELOPPEMENT:")
        if early_phase:
            print(f"Phase précoce (t=0-{max_time_available//3}):")
        ...
        if middle_phase:
            print(f"Phase intermédiaire (t={max_time_available//3}-{2*max_time_available//3}):")
...
# ANALYSE DU CAS B_0_15 AVEC TOUTES LES VISUALISATIONS
print("🚀 DÉBUT DE L'ANALYSE COMPLÈTE DU CAS B_0_15")

# Cas B_0_15 (cas unique, pas de sous-dossiers x/y)
case_015_path = r'C:\Users\user\Desktop\Donnees_KVI\Kelvin_Helmholtz_Instabilities_B_0_15\MHD_data_kelvin_helmholtz_B_0_15'
results_015 = analyze_kh_case_with_plots('B_0_15', case_015_path, max_time=35)

# RÉSUMÉ FINAL
print(f"\n{'='*80}")
print(f"=== RÉSUMÉ FINAL DU CAS B_0_15 ===")
print(f"{'='*80}")

if results_015:
    print(f"\n🎯 CARACTÉRISTIQUES DU CAS B_0_15:")
    print(f"  - Champ magnétique: B₀ = 0.15 (INTERMÉDIAIRE)")
    print(f"  - Configuration: CAS UNIQUE (pas de directionnalité x/y)")
    print(f"  - Fichiers analysés: {len(results_015['all_times']) if results_015['all_times'] else 0}")

    if results_015['evolution_metrics']:
        final_metrics = results_015['evolution_metrics'][-1]
        initial_metrics = results_015['evolution_metrics'][0]

        print(f"\n📊 MÉTRIQUES FINALES:")
        print(f"  - Singularités finales: {final_metrics['n_singularities']}")
        print(f"  - Intensité maximale: {final_metrics['strongest_singularity']:.4f}")
        print(f"  - Énergie ondelettes: {final_metrics['wavelet_energy']:.2e}")

        print(f"\n📈 ÉVOLUTION:")
        print(f"  - Croissance singularités: {initial_metrics['n_singularities']} → {final_metrics['n_singularities']}")
        print(f"  - Croissance intensité: {initial_metrics['strongest_singularity']:.4f} → {final_metrics['strongest_singularity']:.4f}")

        if len(results_015['evolution_metrics']) > 1:
            energy_growth = final_metrics['wavelet_energy'] / initial_metrics['wavelet_energy']
            print(f"  - Croissance énergie: ×{energy_growth:.1f}")

    print(f"\n🔬 IMPLICATIONS PHYSIQUES:")
    print(f"  - Champ magnétique INTERMÉDIAIRE (B₀ = 0.15)")
    print(f"  - Position entre stabilisation (B₀=0.1) et résonance (B₀=0.2)")
    print(f"  - Comportement de transition critique")
    print(f"  - Clé pour comprendre la courbe de stabilité magnétique")

else:
    print("❌ Échec de l'analyse du cas B_0_15")

print(f"\n✅ ANALYSE COMPLÈTE AVEC VISUALISATIONS TERMINÉE POUR LE CAS B_0_15")
